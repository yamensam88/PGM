...
export default async function HumanResourcesPage(props: { searchParams: Promise<{ filter?: string, from?: string, to?: string }> }) {
  ...
  const todayRunsData = await prisma.dailyRun.findMany({
    where: {
  ...
  const todayRunsDriverIds = new Set(todayRunsData.map(r => r.driver_id));
  ...
  const periodRunsCalendar = await prisma.dailyRun.findMany({
    where: {
      organization_id: orgId,
      date: { gte: startDate, lte: endDate },
