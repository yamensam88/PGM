...
export default async function DriverDeliveriesPage(props: { searchParams: Promise<{ edit?: string, driverId?: string }> }) {
  ...
  if (currentRun?.status === 'completed' && !isEditing) {
    ...
    return (
      <main className="flex-1 mt-10 md:mt-16 flex flex-col items-center justify-center text-center">
        ...
        {isToday ? (
           <a href="?edit=true" className="mt-8 inline-flex items-center justify-center px-6 py-2.5 bg-zinc-900 text-white rounded-xl font-medium text-sm hover:bg-zinc-800 transition-colors shadow-sm">
  ...
  return (
      <main className="flex-1 w-full pt-2">
        ...
        {isEditing && (
          <div className="mb-6 p-4 bg-teal-50 border border-teal-200 text-teal-800 rounded-2xl text-sm font-medium shadow-sm">
            <strong>Mode Édition :</strong> Vous modifiez la tournée soumise aujourd'hui.
          </div>
        )}
        
        <h2 className="text-[11px] font-bold tracking-widest uppercase text-zinc-500 mb-3 ml-2">Saisie du Bordereau</h2>
        <UnifiedDeliveryForm 
           driverId={driver.id} 
           vehicles={vehicles} 
           clients={clients}
