...
import { recordDriverPenalty } from "@/lib/actions";
...
export function CreatePenaltyForm({ drivers, onSuccess }: CreatePenaltyFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      try {
        const result = await recordDriverPenalty(formData);
        if (result.success) {
    ...
    }); // Automatically triggers revalidatePath
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="grid gap-5 py-4">
      <div className="grid grid-cols-4 items-center gap-4">
        ...
        >
          <option value="" disabled selected>Sélectionnez le chauffeur responsable</option>
          {drivers.map(driver => (
      ...
      <div className="grid grid-cols-4 items-center gap-4">
        ...
        <div className="col-span-3 relative">
          <Input id="amount" name="amount" type="number" step="0.01" min="0" required placeholder="Ex: 50.00" className="pl-8 bg-white border-slate-200 text-slate-700 focus-visible:ring-zinc-600" />
      ...
      <div className="grid grid-cols-4 items-center gap-4">
        ...
        <Input id="date" name="date" type="date" required defaultValue={new Date().toISOString().split('T')[0]} className="col-span-3 bg-white border-slate-200 text-slate-700 focus-visible:ring-zinc-600 [color-scheme:dark]" />
      </div>

      <div className="grid grid-cols-4 items-start gap-4">
        <Label htmlFor="description" className="text-right font-medium mt-2 text-slate-600 text-[13px]">Motif <span className="text-slate-500 font-normal ml-1">(Optionnel)</span></Label>
        <Textarea 
          id="description" 
          name="description" 
      ...
      <div className="flex justify-end gap-3 mt-4">
        ...
        <Button type="submit" disabled={isPending} className="bg-red-600 text-slate-900 hover:bg-red-700 shadow-sm shadow-red-900/20">
          {isPending ? "Enregistrement..." : "Enregistrer la pénalité"}
        </Button>
      </div>
    </form>
  );
